...
def main():
    ...
    for trade_window_start in [starting_month + pd.DateOffset(months=i) for i in range(0, total_months, trade_months)]:
        ...
        print(f"Finished collecting data, beginning to train contenders.")

        processes = []
        for i in range(int(num_contenders)):
            p = multiprocessing.Process(target=ModelTools.train_PPO, args=(train_data, test_data, training_rounds_per_contender, PPO_Contenders,))
            p.start()
            processes.append(p)
